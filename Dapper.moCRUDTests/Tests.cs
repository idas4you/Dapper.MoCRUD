...
namespace Dapper.MoCRUDTests
{
    ...
    public class Tests
    {
        public Tests(MoCRUD.Dialect dbtype)
        {
            _dbtype = dbtype;
        }
        private MoCRUD.Dialect _dbtype;

        private IDbConnection GetOpenConnection()
        {

            IDbConnection connection;
            if (_dbtype == MoCRUD.Dialect.PostgreSQL)
            {
            ...
            else if (_dbtype == MoCRUD.Dialect.SQLite)
            {
                connection = new SQLiteConnection("Data Source=MyDatabase.sqlite;Version=3;");
                MoCRUD.SetDialect(MoCRUD.Dialect.SQLite);
            }
            else if (_dbtype == MoCRUD.Dialect.MySQL)
            {
            ...
            else
            {
                ...
                MoCRUD.SetDialect(MoCRUD.Dialect.SQLServer);
            }
        ...
        {
            MoCRUD.SetDialect(MoCRUD.Dialect.SQLServer);
            MoCRUD.GetDialect().IsEqualTo(MoCRUD.Dialect.SQLServer.ToString());
            MoCRUD.SetDialect(MoCRUD.Dialect.PostgreSQL);
            MoCRUD.GetDialect().IsEqualTo(MoCRUD.Dialect.PostgreSQL.ToString());
        }
